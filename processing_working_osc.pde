...
void setup() {
  ...
  cp5.addButton("Off", 1.0, width -90 ,20,70,70)
     .setColorForeground(color(180,5,20)) 
     .setColorBackground(color(150,0,10)) 
     .setColorActive(color(200,15,20)) 
     
     .getCaptionLabel().setFont(knobFont).setColor(color(255)); 
...
void controlEvent(ControlEvent theEvent) {
  if (theEvent.isController()) {
    ...
    // Gestisci gli eventi dei knob
      ...
      // Valore Bottone 
      else if (theEvent.getController().getName().equals("Off")) {
        msg = new OscMessage("/"+theEvent.getController().getName()); // Cambia il nome del messaggio OSC
        msg.add(theEvent.getController().getValue());
        oscP5.send(msg, superColliderAddr);
        delay(100);
